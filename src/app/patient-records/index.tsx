...
export default function PatientRecordsScreen() {
  ...
  const loadPatients = async (showRefresh = false) => {
    ...
    try {
      ...
      const formattedData = apiData.map((p: any) => ({
        ...
        gender: p.gender
          ? ((p.gender.charAt(0).toUpperCase() + p.gender.slice(1)) as
              | "Male"
              | "Female")
          : "Female",
  ...
  const handleSaveSubmit = async () => {
    ...
    if (!activeClinic) {
      Alert.alert(
        "No Clinic Selected",
        "Please select a clinic before adding a patient.",
      );
      return;
    }
    ...
    try {
      if (editingPatientId) {
        ...
        Alert.alert(
          "Success",
          `Patient data for ${firstName} ${lastName} has been updated.`,
          [
            {
              text: "OK",
              onPress: () => {
        ...
        Alert.alert(
          "Patient Added",
          `Record saved for ${firstName} ${lastName}.`,
          [
            {
              text: "OK",
              onPress: () => {
